  ...
  */
  ...
  async function marcarCompletado() {
    ...
    try {
      ...
      // 3. Obtener la sesión del profesional para usarla
      // cuando termine la ventana de prueba.
      const {
        data: sessionData,
        error: sessionError,
      ...
      if (sessionError || !sessionData.session) {
        throw new Error(
          "El trabajo se completó, pero no pudimos preparar la liberación del pago."
        );
      }
      ...
      // 4. PRUEBA TEMPORAL:
      // Esperar 5 minutos antes de intentar liberar el pago.
      // La ruta /api/payments/release comprobará primero
      // si existe un reclamo open/reviewing.
      //
      // IMPORTANTE:
      // Esta solución requiere mantener esta página abierta
      // durante la prueba. Después la cambiaremos por un
      // proceso automático del servidor para las 48 horas.
      window.setTimeout(async () => {
        try {
          console.log(
            "Terminó la ventana de prueba de 5 minutos. Comprobando liberación..."
          );
          ...
          if (!releaseResponse.ok) {
            if (releaseData?.paymentBlocked) {
              console.log(
                "PAGO RETENIDO POR RECLAMO:",
                releaseData
              );

              setMensaje(
                "Trabajo completado. El pago continúa retenido porque existe un reclamo activo."
              );
            ...
            console.error(
              "No se pudo liberar el pago después de 5 minutos:",
              releaseData
            );

            setError(
              releaseData?.error ||
                "No se pudo liberar el pago después del período de prueba."
            );
          ...
          console.log(
            "PAGO LIBERADO DESPUÉS DE 5 MINUTOS:",
            releaseData.stripeTransferId
          );

          setMensaje(
            `Período de revisión finalizado. Pago de $${Number(
              releaseData.providerNetAmount
            ).toFixed(2)} liberado correctamente.`
          );

          await cargarTodo();
        } catch (releaseError) {
          console.error(
            "Error en liberación programada:",
            releaseError
          );
